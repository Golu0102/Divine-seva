...
<section id="services" class="py-20 bg-white">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="section-subtitle">
            {{ $setting->blog_3 ?? 'We offer a wide range of poojas and ceremonies performed by our learned Pandits, adhering to Vedic traditions.' }}
        </p>
        <div class="overflow-hidden mt-12">
            <div id="carousel-wrapper" class="flex space-x-4 overflow-x-auto scroll-smooth whitespace-nowrap">
                @foreach ($poojas as $pooja)
                    <div class="min-w-[300px] bg-amber-50 rounded-xl p-6 text-center shadow-lg inline-block transition-all duration-300 hover:shadow-2xl hover:border hover:border-brand-orange">
                        <div class="flex justify-center items-center h-20 w-20 rounded-full bg-brand-orange/10 mx-auto">
                            <img src="{{ asset('storage/' . $pooja->image) }}" class="rounded-full w-12 h-12 object-cover" alt="Pooja Icon">
                        </div>
                        ...
                        <a href="{{ route('pooja.view', $pooja->id) }}" class="text-brand-orange font-semibold mt-4 inline-block">Book Now &rarr;</a>
                    </div>
                @endforeach
                {{-- Duplicate --}}
                @foreach ($poojas as $pooja)
...
<a href="tel:{{ $setting->mobile ?? '+911234567890' }}" class="floating-button call-button">
    <div class="floating-tooltip">Call us</div>
    <i class="fas fa-phone-alt"></i>
</a>

<!-- Floating WhatsApp Button -->
<a href="https://wa.me/{{ $setting->mobile ?? '911234567890' }}" class="floating-button whatsapp-button" target="_blank">
