<footer id="contact" class="bg-brand-maroon text-white">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="md:col-span-1">
                <a href="{{ url('/') }}" class="flex items-center space-x-2">
                    <img src="{{ asset($setting->logo ?? 'https://placehold.co/40x40/FF6D2E/FFFFFF?text=DS') }}"
                         alt="Logo"
                         class="rounded-full w-10 h-10 object-cover">
                    <span class="text-2xl font-laila font-bold text-white">
                        {{ $setting->site_name ?? 'Divine Seva' }}
                    </span>
                </a>
                <p class="mt-4 text-gray-300">
                    {{ $setting->blog_7 ?? 'Trusted partner for authentic Vedic poojas.' }}
                </p>

                <div class="mt-6 flex space-x-4">
                    @if (!empty($setting->facebook))
                        <a href="{{ $setting->facebook }}" target="_blank" class="text-gray-300 hover:text-brand-orange">
                            <i data-lucide="facebook"></i>
                        </a>
                    @endif
                    @if (!empty($setting->instagram))
                        <a href="{{ $setting->instagram }}" target="_blank" class="text-gray-300 hover:text-brand-orange">
                            <i data-lucide="instagram"></i>
                        </a>
                    @endif
                    @if (!empty($setting->youtube))
                        <a href="{{ $setting->youtube }}" target="_blank" class="text-gray-300 hover:text-brand-orange">
                            <i data-lucide="youtube"></i>
                        </a>
                    @endif
                    @if (!empty($setting->twitter))
                        <a href="{{ $setting->twitter }}" target="_blank" class="text-gray-300 hover:text-brand-orange">
            ...
            <div>
                ...
                <ul class="mt-4 space-y-2">
                    ...
                    <li><a href="{{ url('/') }}#pandits" class="text-gray-300 hover:text-white">Our Pandits</a></li>
            ...
            <div>
                ...
                <ul class="mt-4 space-y-3 text-gray-300">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="tel:{{ $setting->mobile ?? '+911234567890' }}"
                           class="hover:text-white">
                            {{ $setting->mobile ?? '+91 12345 67890' }}
                        </a>
                    </li>
                    <li class="flex items-center">
                        <i data-lucide="mail" class="w-5 h-5 mr-3"></i>
                        <a href="mailto:{{ $setting->email ?? 'seva@divine.com' }}"
                           class="hover:text-white">
                            {{ $setting->email ?? 'seva@divine.com' }}
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="mt-12 border-t border-gray-600 pt-8 text-center text-gray-400">
            <p>
                &copy; {{ now()->year }} {{ $setting->site_name ?? 'Divine Seva' }}.
